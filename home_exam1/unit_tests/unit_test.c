...
int unit_test() {
  ...
  int test_mutual_links2;
  int *num_involvements2;
  alloc1D(&num_involvements2, N1);
  test_mutual_links2 = count_mutual_links1_openMP(N1, table2D, num_involvements2);
  check3 = check_1Dvector(num_involvements2, correct_vector, N1);
  if (check3 == 1 || (test_mutual_links2 != 13)){
      printf("count_mutual_links1_openMP is NOT correct\n");
      return 1;
  }
  ...
  start_MP = clock();
  test_mutual_links4 = count_mutual_links2_openMP(N2, N_links, row_ptr, col_idx, num_involvements4);
  end_MP = clock();

  check7 = check_1Dvector(num_involvements4, correct_vector, N2-1);
  if (check7 == 1 || (test_mutual_links4 != 13)){
      printf("count_mutual_links2_openMP is NOT correct\n");
      return 1;
  }
